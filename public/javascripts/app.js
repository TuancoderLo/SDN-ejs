...
function setupEventListeners() {
    ...
    document.getElementById('perfumeForm')?.addEventListener('submit', handlePerfumeSubmit);
    document.getElementById('editBrandForm')?.addEventListener('submit', updateBrandFromModal);
    ...
    document.getElementById('loginLinkMobile')?.addEventListener('click', function(e) {
        console.log('🔗 Mobile login link clicked, storing URL:', window.location.href);
        const success = storeRedirectUrl(window.location.href);
        console.log('Storage result:', success);
    });
    document.getElementById('registerLink')?.addEventListener('click', function(e) {
        console.log('🔗 Register link clicked, storing URL:', window.location.href);
        const success = storeRedirectUrl(window.location.href);
        console.log('Storage result:', success);
    });
    document.getElementById('registerLinkMobile')?.addEventListener('click', function(e) {
        console.log('🔗 Mobile register link clicked, storing URL:', window.location.href);
        const success = storeRedirectUrl(window.location.href);
        console.log('Storage result:', success);
    });
    ...
    if (token && userString) {
        ...
        if (user) {
            ...
            if (user.isBlocked) {
                ...
                const blockDate = user.blockedAt ? new Date(user.blockedAt).toLocaleDateString() : 'Unknown';
                showNotification(`🚫 Your account has been blocked. Reason: ${blockReason}. Blocked on: ${blockDate}`, 'error');
    ...
    try {
        ...
        if (resp.ok && data && data.member) {
            ...
            if (data.message === 'Account is blocked') {
                ...
                const blockDate = data.blockedAt ? new Date(data.blockedAt).toLocaleDateString() : 'Unknown';
                showNotification(`🚫 Your account has been blocked. Reason: ${blockReason}. Blocked on: ${blockDate}`, 'error');
                // Clear stored data and redirect to login
                localStorage.removeItem('token');
                localStorage.removeItem('user');
...
async function handleRegister(e) {
    ...
    if (!emailElement || !passwordElement || !nameElement || !confirmPasswordElement || !yobElement || !genderElement) {
...
async function loadUserProfile() {
    ...
    if (profileGender) profileGender.value = currentUser.gender ? 'true' : 'false';
}
...
function showLoadingSkeleton() {
    ...
    grid.innerHTML = `
        ${Array.from({length: 8}, (_, i) => `
            <div class="card luxury-glass premium-shadow animate-pulse border border-white/20 rounded-3xl overflow-hidden" style="animation-delay: ${i * 0.1}s;">
...
function displayPerfumes(perfumes) {
    ...
    grid.innerHTML = perfumes.map((perfume, index) => `
        <div class="group card bg-white/90 backdrop-blur-sm shadow-xl hover:shadow-2xl transition-all duration-700 cursor-pointer transform hover:-translate-y-4 hover:scale-105 border border-gray-200 rounded-3xl overflow-hidden animate-fadeInUp" 
             style="animation-delay: ${index * 0.1}s;" 
             onclick="goToPerfumeDetail('${perfume._id}')">
            ...
            <figure class="relative overflow-hidden bg-gradient-to-br from-white/10 to-white/5">
                <div class="relative h-64 w-full">
                    <img src="${perfume.imageUrl || perfume.uri || ''}" 
                         ...
                         class="w-full h-full object-contain transition-all duration-700 group-hover:scale-110 group-hover:brightness-105 p-4"
                    ...
                    <div class="absolute top-4 left-4">
                        <div class="badge badge-sm shadow-lg font-semibold text-black bg-yellow-200 border border-yellow-300">
                    ...
                    <div class="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-2 group-hover:translate-y-0">
                        <div class="badge badge-lg shadow-lg font-bold text-black bg-green-200 border border-green-300">
                    ...
                    <!-- Concentration Badge -->
                    <div class="absolute bottom-4 left-4 opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-2 group-hover:translate-y-0">
                        <div class="badge badge-sm shadow-lg font-medium text-black bg-blue-200 border border-blue-300">
                    ...
                    <div class="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-center justify-center">
                        <div class="badge badge-lg shadow-xl text-white luxury-gradient">
            ...
            <div class="card-body p-6 space-y-4">
                <!-- Product Name -->
                <h2 class="card-title text-xl font-bold text-gray-800 group-hover:text-blue-600 transition-colors duration-500 leading-tight text-center">
                    ${perfume.perfumeName || 'Unknown Perfume'}
                </h2>
                
                <!-- Brand Name -->
                <div class="text-center">
                    <button class="text-gray-600 font-semibold text-base hover:text-blue-600 transition-colors duration-300 underline decoration-2 underline-offset-2 ${perfume.brand?.isDeleted ? 'opacity-60' : ''}" 
                            onclick="event.stopPropagation(); selectBrandFilter('${perfume.brand?._id || ''}')">
                        ${perfume.brand ? perfume.brand.brandName : 'Unknown Brand'}${perfume.brand?.isDeleted ? ' (Deleted)' : ''}
                    </button>
                </div>
                
                <!-- Gender Badge -->
                <div class="flex justify-center">
                    <div class="badge badge-lg shadow-lg group-hover:scale-110 transition-transform duration-300 font-semibold text-black border" 
                         style="background: ${perfume.targetAudience === 'male' ? '#e0e7ff' : perfume.targetAudience === 'female' ? '#fce7f3' : '#f3f4f6'}; border-color: ${perfume.targetAudience === 'male' ? '#c7d2fe' : perfume.targetAudience === 'female' ? '#f9a8d4' : '#d1d5db'};">
                        <i class="fas ${perfume.targetAudience === 'male' ? 'fa-mars' : perfume.targetAudience === 'female' ? 'fa-venus' : 'fa-venus-mars'} mr-2"></i>
                        ${perfume.targetAudience || 'Unknown'}
                    </div>
                </div>
                
                <!-- Volume Info -->
                <div class="flex items-center justify-center text-sm text-gray-700 font-medium">
                    <i class="fas fa-flask mr-2 text-yellow-600"></i>
                    <span>${perfume.volume || '100'}ml</span>
                </div>
                
                <!-- Decorative Elements -->
                <div class="flex justify-center items-center space-x-2 mt-4">
                    <div class="w-1 h-1 bg-gold rounded-full"></div>
                    <div class="w-2 h-2 bg-platinum rounded-full"></div>
                    <div class="w-1 h-1 bg-gold rounded-full"></div>
                </div>
                ...
                <div class="card-actions justify-center mt-4 gap-2">
                    <button class="btn btn-sm gold-gradient hover:emerald-gradient text-white shadow-lg" onclick="event.stopPropagation(); goToPerfumeDetail('${perfume._id}')">
                        <i class="fas fa-eye mr-1"></i>
                        View
                    </button>
                    <button class="btn btn-sm sapphire-gradient hover:ruby-gradient text-white shadow-lg" onclick="event.stopPropagation(); editPerfume('${perfume._id}')">
        ...
        if (response.ok) {
            ...
            detail.innerHTML = `
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div>
                        <img src="${perfume.uri || 'https://via.placeholder.com/400x400?text=No+Image'}" 
                    ...
                    <div class="space-y-6">
                        <div>
                            <h1 class="text-4xl font-bold mb-2">${perfume.perfumeName}</h1>
                            <p class="text-2xl text-primary font-semibold ${perfume.brand?.isDeleted ? 'opacity-60' : ''}">
                                ${perfume.brand ? perfume.brand.brandName : 'Unknown Brand'}${perfume.brand?.isDeleted ? ' (Deleted)' : ''}
                            </p>
                        </div>
                        
                        <div class="flex flex-wrap gap-2">
                            <div class="badge badge-primary badge-lg">${perfume.concentration || 'Unknown'}</div>
                            <div class="badge badge-secondary badge-lg">${perfume.targetAudience || 'Unknown'}</div>
                            <div class="badge badge-accent badge-lg">${perfume.volume || 'Unknown'}ml</div>
                            <div class="badge badge-info badge-lg">$${perfume.price || 'Unknown'}</div>
                        </div>
                        
                        <div>
                            <h3 class="text-xl font-bold mb-2">Description</h3>
                            <p class="text-base-content/80">${perfume.description || 'No description available'}</p>
                        </div>
                        
                        <div>
                            <h3 class="text-xl font-bold mb-2">Ingredients</h3>
                            <p class="text-base-content/80">${perfume.ingredients || 'No ingredients listed'}</p>
                        </div>
                    </div>
                </div>
                                        ...
                                        <div class="flex items-center gap-3 mb-2">
                                            <div class="avatar placeholder">
                                                <div class="bg-neutral text-neutral-content rounded-full w-8">
                                                    <span class="text-xs">${comment.author ? comment.author.name.charAt(0).toUpperCase() : 'U'}</span>
                                                </div>
                                            </div>
                                            <div>
                                                <div class="font-semibold">${comment.author ? comment.author.name : 'Anonymous'}</div>
                                                <div class="flex items-center gap-2">
                                                    <div class="rating rating-sm">
                                                        ${[1,2,3,4,5].map(i => `<input type="radio" class="mask mask-star-2 bg-orange-400" ${i <= comment.rating ? 'checked' : ''} disabled />`).join('')}
                                                    </div>
                                                    <span class="text-sm text-base-content/60">${new Date(comment.createdAt).toLocaleDateString()}</span>
                                                </div>
                                            </div>
                                        </div>
...
function filterPerfumes() {
    ...
    if (searchTerm) {
        filtered = filtered.filter(p => 
            ...
            (p.brand?.isDeleted && `deleted ${p.brand.brandName}`.toLowerCase().includes(searchTerm))
        );
    }
...
function updateBrandFilters() {
    ...
    const activeOptions = activeBrands.map(brand => 
        `<option value="${brand._id}">${brand.brandName}</option>`
    ).join('');
    
    const deletedOptions = deletedBrands.map(brand => 
    ...
    if (brandFilter) {
    brandFilter.innerHTML = '<option value="">All Brands</option>' + activeOptions + deletedOptions;
    ...
    if (perfumeBrandSelect) {
        ...
        perfumeBrandSelect.innerHTML = '<option value="">Chọn thương hiệu</option>' + activeOptions + deletedOptions;
        console.log('Updated perfume brand dropdown with', activeBrands.length + deletedBrands.length, 'brands');
    } else {
...
async function loadAllAdminData() {
    try {
        ...
        const [membersResult, brandsResult, perfumesResult] = await Promise.allSettled([
            loadMembers(),
            loadAdminBrands(), 
            loadAdminPerfumes()
        ]);
...
async function showAdminTab(tab) {
    try {
        ...
        const tabButton = document.querySelector(`[onclick="showAdminTab('${tab}')"]`);
        if (tabButton) {
...
async function loadMembers() {
    try {
        ...
        if (response.ok) {
            console.log('Loaded all members:', users.length, users.map(u => ({ name: u.name, email: u.email, isBlocked: u.isBlocked })));
                ...
                <div class="mb-4 p-4 bg-gray-50 rounded-lg">
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex gap-4">
                            <span class="badge badge-success">Active: ${activeMembers.length}</span>
                            <span class="badge badge-error">Blocked: ${blockedMembers.length}</span>
                    ...
                    <table class="table table-zebra w-full">
                        ...
                        <tbody>
                            ${users.map(user => {
                                ...
                                return `
                                    <tr class="${isBlocked ? 'opacity-60 bg-red-50' : ''}">
                                        <td class="font-semibold ${isBlocked ? 'text-red-600' : ''}">${user.name}</td>
                                        <td>${user.email}</td>
                                        <td>${user.YOB}</td>
                                        ...
                                        <td>
                                            <div class="flex gap-2">
                                                ${actionButtons}
                                                ${isBlocked ? `<button class="btn btn-info btn-sm" onclick="viewMemberDetails('${user._id}')">View</button>` : ''}
                                            </div>
                                        </td>
                                    </tr>
                                `;
...
async function unblockMember(id) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const errorData = await response.json();
            showNotification(errorData.message || 'Failed to unblock member', 'error');
        }
    } catch (error) {
            ...
            <div class="modal-box max-w-md">
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <p>${member.isBlocked ? '<span class="badge badge-error">Blocked</span>' : '<span class="badge badge-success">Active</span>'}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="text-red-600">${member.blockReason || 'No reason provided'}</p>
                    </div>
                    ` : ''}
                </div>
                <div class="modal-action">
                    <button class="btn btn-primary" onclick="closeModal()">Close</button>
                    ${member.isBlocked ? `<button class="btn btn-success" onclick="unblockMember('${member._id}'); closeModal();">Unblock Member</button>` : ''}
                </div>
            </div>
        `;
...
function setActiveSlide(slideNumber) {
    ...
    const targetBtn = document.querySelector(`#carouselDots .carousel-btn:nth-child(${slideNumber})`);
    if (targetBtn) {
...
function startAutoRotate() {
    const total = document.querySelectorAll('#featuredCarousel .carousel-item').length || 3;
...
function renderFeaturedCarousel(perfumes) {
    ...
    // Build slides
    const slidesHtml = perfumes.map((p, idx) => {
        const n = idx + 1;
        const g = genderBadge(p.targetAudience);
        const brandName = p.brand && (p.brand.brandName || p.brandName) ? (p.brand.brandName || p.brandName) : '';
        return `
                ...
                <div class="card w-full bg-gradient-to-br from-primary/10 to-secondary/10 shadow-2xl">
                    <div class="card-body flex-row items-center p-8">
                        <div class="flex-1 text-left">
                            <h2 class="card-title text-3xl mb-4">${p.perfumeName || 'Unknown Perfume'}</h2>
                            <p class="text-sm opacity-70 mb-2 ${p.brand?.isDeleted ? 'line-through text-red-500' : ''}">${brandName}${p.brand?.isDeleted ? ' (Deleted)' : ''}</p>
                            <p class="text-lg mb-4 line-clamp-3">${p.description || ''}</p>
                            <div class="flex items-center gap-4">
                                <div class="badge badge-primary badge-lg">${p.concentration || 'EDP'}</div>
                                <div class="badge ${g.cls} badge-lg">${g.text}</div>
                                <div class="text-2xl font-bold text-primary">${p.price ? '$' + p.price : ''}</div>
                            </div>
                        </div>
                        <div class="flex-shrink-0 ml-8">
                            <img src="${p.uri || 'https://via.placeholder.com/300x300?text=No+Image'}"
                                 alt="${p.perfumeName || 'Perfume'}" class="w-48 h-48 object-cover rounded-2xl shadow-lg"
    ...
    // Build dots
    const dotsHtml = perfumes.map((_, idx) => {
        const n = idx + 1;
        return `<a href="#slide${n}" class="btn btn-sm btn-circle carousel-btn ${n === 1 ? 'active' : ''}" onclick="setActiveSlide(${n})">${n}</a>`;
    }).join('');
...
function showNotification(message, type = 'info') {
    ...
    toast.className = 'alert shadow-2xl border border-base-200/50 backdrop-blur-md animate-fadeInUp relative';
    if (type === 'success') {
...
async function handleCommentSubmit(e) {
    ...
    try {
        const response = await fetch(`${API_URL}/perfumes/${currentPerfumeId}/comments`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${safeGetFromStorage('token')}`
...
async function handlePerfumeSubmit(e) {
    ...
    const requiredElements = [
        'perfumeBrand', 'perfumeName', 'perfumePrice', 'perfumeVolume',
        'perfumeConcentration', 'perfumeTarget', 'perfumeUri', 'perfumeImageUrl',
        'perfumeDescription', 'perfumeIngredients'
    ];
    ...
    if (selectedBrand && selectedBrand.isDeleted) {
        if (!confirm(`Warning: You are creating a perfume for a deleted brand "${selectedBrand.brandName}". Do you want to continue?`)) {
            return;
        }
    }
    ...
    try {
        const url = isEditMode ? `${API_URL}/perfumes/${perfumeId}` : `${API_URL}/perfumes`;
        ...
        if (response.ok) {
            const message = isEditMode ? 'Perfume updated successfully!' : 'Perfume added successfully!';
            ...
            const message = isEditMode ? 'Failed to update perfume' : 'Failed to add perfume';
            showNotification(data.message || message, 'error');
        }
    } catch (error) {
        const message = isEditMode ? 'Error updating perfume' : 'Error adding perfume';
...
async function loadAdminBrands() {
    try {
        ...
        if (response.ok) {
            console.log('Loaded brands for admin table:', brands.map(b => ({
                _id: b._id,
                name: b.brandName,
            ...
            const problemBrand = brands.find(b => b._id === '68f4e8c88779ae0d01ed5dc0');
            if (problemBrand) {
            ...
            const activeBrands = brands.filter(brand => !brand.isDeleted && !brand.deletedAt);
            const deletedBrands = brands.filter(brand => brand.isDeleted || brand.deletedAt);
...
async function loadAdminPerfumes() {
    try {
        ...
        if (response.ok) {
            console.log('Loaded perfumes for admin table:', perfumes.map(p => ({
                name: p.perfumeName,
                imageUrl: p.imageUrl,
                uri: p.uri,
                        ...
                        <tbody>
                            ${perfumes.map(perfume => `
                                <tr>
                                    ...
                                    <td class="font-semibold">${perfume.perfumeName}</td>
                                    <td class="${perfume.brand?.isDeleted ? 'opacity-60 text-red-600' : ''}">
                                        ${perfume.brand ? perfume.brand.brandName : 'Unknown'}${perfume.brand?.isDeleted ? ' (Deleted)' : ''}
                                    </td>
                                    <td>
                                        <span class="badge badge-soft">$${perfume.price}</span>
                                    </td>
                                    <td>
                                        <div class="badge ${perfume.targetAudience === 'male' ? 'badge-primary' : perfume.targetAudience === 'female' ? 'badge-secondary' : 'badge-accent'}">
                                            ${perfume.targetAudience}
                                        </div>
                                    </td>
                                    <td>
                                        <div class="flex gap-2">
                                            <button class="btn btn-outline btn-secondary btn-sm" onclick="editPerfume('${perfume._id}')">
                                                <i class="fas fa-edit mr-1"></i>
                                                Edit
                                            </button>
                                            <button class="btn btn-outline btn-error btn-sm" onclick="deletePerfume('${perfume._id}')">
...
function filterBrands(filter) {
    ...
    switch(filter) {
        case 'active':
            filteredBrands = allAdminBrands.filter(brand => !brand.isDeleted && !brand.deletedAt);
            break;
        case 'deleted':
            filteredBrands = allAdminBrands.filter(brand => brand.isDeleted || brand.deletedAt);
            break;
    ...
    table.innerHTML = `
        <div class="mb-4 p-4 bg-gray-50 rounded-lg">
            <div class="flex justify-between items-center">
                ...
                <div class="flex gap-4">
                    <span class="badge badge-success">Active: ${activeBrands.length}</span>
                    <span class="badge badge-error">Deleted: ${deletedBrands.length}</span>
                    <span class="badge badge-info">Total: ${brands.length}</span>
                </div>
            </div>
            ...
            <table class="table table-zebra w-full">
                ...
                <tbody>
                    ${brands.map(brand => {
                        ...
                        return `
                            <tr class="${isDeleted ? 'opacity-60 bg-red-50' : ''}">
                                <td class="font-semibold ${isDeleted ? 'text-red-600' : ''}">${brand.brandName}</td>
                                <td>${statusBadge}</td>
                                ...
                                <td>${deletedDate}</td>
                                        <td>
                                            <div class="flex gap-2">
                                                ${valueButton}
                                                ${!isDeleted ? `<button class="btn btn-info btn-sm" onclick="editBrandName('${brand._id}', '${brand.brandName}')">
                                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
async function deleteBrand(id) {
    ...
    try {
        ...
        if (response.ok) {
            const data = await response.json();
            if (data.softDeleted) {
                showNotification(`Brand unvalued (has ${data.productCount} products)`, 'warning');
            } else {
...
async function loadAdminProfile() {
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById('profileYOB').value = userData.YOB || userData.yob || '';
            document.getElementById('profileGender').value = userData.gender || 'true';
